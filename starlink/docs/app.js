...
(() => {
  ...
  function eciToGeodetic(eci, gmst2) {
    var a = 6378.137;
    var b = 6356.7523142;
    var R = Math.sqrt(eci.x * eci.x + eci.y * eci.y);
    var f = (a - b) / a;
    var e2 = 2 * f - f * f;
    var longitude = Math.atan2(eci.y, eci.x) - gmst2;
    while (longitude < -pi) {
      longitude += twoPi;
    }
    while (longitude > pi) {
  ...
  function calculateGroundDistance(satLat, satLon, satAlt, obsLat, obsLon) {
    const groundDist = haversineDistance(satLat, satLon, obsLat, obsLon);
    const distance = Math.sqrt(groundDist ** 2 + satAlt ** 2);
    return distance;
  }
  function calculateElevation(satLat, satLon, satAlt, obsLat, obsLon) {
    const groundDist = haversineDistance(satLat, satLon, obsLat, obsLon);
    if (groundDist < 0.1) {
      return 90;
    }
  ...
  function calculateAzimuth(satLat, satLon, obsLat, obsLon) {
    ...
    const x = Math.cos(obsLatRad) * Math.sin(satLatRad) - Math.sin(obsLatRad) * Math.cos(satLatRad) * Math.cos(dlonRad);
  ...
  function gmst(jd) {
    ...
    let gmstDeg = 280.46061837 + 360.98564736629 * (jd - 2451545) + 387933e-9 * T * T;
    gmstDeg = gmstDeg % 360;
    if (gmstDeg < 0)
      gmstDeg += 360;
  ...
  // src/orbit/solar.ts
  var EARTH_RADIUS_KM2 = 6371;
  function calculateSolarPosition(date, lat, lon) {
    const jd = julianDate(date);
    const n = jd - 2451545;
    const L = (280.46 + 0.9856474 * n) % 360;
    const g = degToRad((357.528 + 0.9856003 * n) % 360);
    const lambdaSun = L + 1.915 * Math.sin(g) + 0.02 * Math.sin(2 * g);
    const epsilon = degToRad(23.439 - 4e-7 * n);
    ...
    const sinAlt = Math.sin(latRad) * Math.sin(deltaRad) + Math.cos(latRad) * Math.cos(deltaRad) * Math.cos(lhaRad);
    const elevation = radToDeg(Math.asin(sinAlt));
    let cosAz = (Math.sin(deltaRad) - Math.sin(latRad) * sinAlt) / (Math.cos(latRad) * Math.cos(Math.asin(sinAlt)));
  ...
  function isSatelliteIlluminated(satAlt, sunElevation) {
    const earthShadowAngle = -Math.asin(EARTH_RADIUS_KM2 / (EARTH_RADIUS_KM2 + satAlt)) * (180 / Math.PI);
    return sunElevation > earthShadowAngle;
  }
  function calculateVisibilityRating(sunElevation, satIlluminated, satElevation) {
    if (!satIlluminated) {
      return {
        rating: 0,
        category: "Poor",
  ...
  // src/orbit/propagator.ts
  var SatellitePropagator = class {
    constructor(tleData) {
      this.satrec = null;
      this.name = tleData.name;
      this.noradId = tleData.norad_id;
      try {
        this.satrec = twoline2satrec(tleData.line1, tleData.line2);
    ...
    getOrbitalElements() {
      ...
      return {
        ...
        period: 2 * Math.PI / this.satrec.no
        // minutes
      };
    }
    /**
     ...
     */
    propagate(date, observerLat, observerLon) {
      if (!this.satrec)
        return null;
      try {
        ...
        const gmst2 = gstime(date);
        const positionGd = eciToGeodetic(positionEci, gmst2);
        const satLat = degreesLat(positionGd.latitude);
        const satLon = degreesLong(positionGd.longitude);
        const satAlt = positionGd.height;
     ...
     */
    calculateVisibility(position, observerLat, observerLon) {
  ...
  // src/orbit/calculator.ts
  var PassCalculator = class {
    constructor(observer, parameters) {
     ...
     */
    filterCandidateSatellites(satellites) {
      const filtered = [];
     ...
     */
    async *findPasses(satellites, startTime, endTime, onProgress) {
     ...
     */
    findSatellitePasses(propagator, startTime, endTime) {
      const passes = [];
      const coarseStep = 5 * 60 * 1e3;
     ...
     */
    refinePass(propagator, approxStart, approxEnd) {
      const fineStep = 10 * 1e3;
      const positions = [];
      const searchStart = new Date(approxStart.getTime() - 2 * 60 * 1e3);
      const searchEnd = new Date(approxEnd.getTime() + 2 * 60 * 1e3);
      let currentTime = searchStart;
      let maxElevation = 0;
      let maxElevationTime = null;
      let maxElevationPosition = null;
      let passStartTime = null;
      let passEndTime = null;
      ...
      return {
        ...
        duration_seconds: Math.round((passEndTime.getTime() - passStartTime.getTime()) / 1e3),
        max_distance_km: Math.round(maxElevationPosition.distance),
        positions: [],
        // Not storing positions to save memory
  ...
  // src/orbit/orbitManager.ts
  var OrbitManager = class {
    constructor() {
      this.tleDataset = null;
     ...
     */
    async calculatePasses(hoursAhead = 24, onProgress) {
      if (!this.tleDataset) {
      ...
      const endTime = new Date(now.getTime() + hoursAhead * 60 * 60 * 1e3);
      this.calculatedPasses = [];
    ...
    updateCalculationProgress(progress) {
      ...
      if (loadingEl) {
        ...
        if (message) {
          ...
          message.textContent = `Lasketaan: ${progress.satellitesProcessed}/${progress.totalSatellites} satelliittia (${percent}%) - ${progress.passesFound} ylilentoa l\xF6ydetty`;
        }
      }
    }
    showError() {
